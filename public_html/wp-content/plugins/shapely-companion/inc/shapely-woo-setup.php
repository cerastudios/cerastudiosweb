...
if ( is_admin() && isset( $_GET['activated'] ) && $pagenow == 'themes.php' ) {
	add_action( 'init', 'shapely_woocommerce_image_dimensions', 1 );
}
/**
 * Define image sizes
 */
...
function shapely_woomenucart( $menu, $args ) {
	...
	ob_start();
	global $woocommerce;
	$viewing_cart        = esc_html( __( 'View your shopping cart', 'shapely' ) );
	$start_shopping      = esc_html( __( 'Start shopping', 'shapely' ) );
	...
	if ( $cart_contents_count == 0 ) {
		$menu_item = '<li class="menu-item"><a class="woo-menu-cart" href="' . esc_url( $shop_page_url ) . '" title="' . esc_attr( $start_shopping ) . '">';
	} else {
		$menu_item = '<li class="menu-item"><a class="woo-menu-cart" href="' . esc_url( $cart_url ) . '" title="' . esc_attr( $viewing_cart ) . '">';
	}

	$menu_item .= '<i class="fa fa-shopping-cart"></i> ';

	$menu_item .= esc_html( $cart_contents ) . ' - ' . esc_html( $cart_total );
	$menu_item .= '</a></li>';
	// Uncomment the line below to hide nav menu cart item when there are no items in the cart
	// }
	echo $menu_item;
